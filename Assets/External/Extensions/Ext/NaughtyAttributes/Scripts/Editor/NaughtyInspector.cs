...
namespace NaughtyAttributes.Editor
{
    ...
    public class NaughtyInspector : UnityEditor.Editor
    {
        ...
        private Dictionary<string, SavedBool> _boxouts = new Dictionary<string, SavedBool>();
        ...
        protected void DrawSerializedProperties()
        {
            ...
            foreach (var group in GetGroupedProperties(_serializedProperties))
            {
                ...
                Rect buttonRect = new Rect(lastRect.x, lastRect.y + EditorGUIUtility.singleLineHeight - 8.5f, lastRect.width, EditorGUIUtility.singleLineHeight + 5.5f);
                if (!_boxouts.ContainsKey(group.Key))
                {
                    _boxouts[group.Key] = new SavedBool($"{target.GetInstanceID()}.{group.Key}", true);
                }
                GUIStyle mystyle = new GUIStyle(GUI.skin.button);
                if (_boxouts[group.Key].Value)
                    mystyle = GUIStyle.none;
                if (GUI.Button(buttonRect, "", mystyle))
                {
                    _boxouts[group.Key].Value = !_boxouts[group.Key].Value;
                }
                ...
                if (_boxouts[group.Key].Value)
                {
                    ...
                    if (_methods.Any())
                    {
                        ...
                        {
                            if (method.GetCustomAttribute<ButtonAttribute>().Name == group.Key)
        ...
        protected void DrawButtons(bool drawHeader = false)
        {
            if (_methods.Any())
            {
                ...
                foreach (var method in _methods)
                {
                    if (method.GetCustomAttribute<ButtonAttribute>().Name == "")
                        NaughtyEditorGUI.Button(serializedObject.targetObject, method);
                }
            }
        }
        ...
        {
            return properties
                .Where(p => PropertyUtility.GetAttribute<BoxGroupAttribute>(p) != null || (PropertyUtility.GetAttribute<ButtonAttribute>(p) != null && PropertyUtility.GetAttribute<ButtonAttribute>(p).Name != ""))
                .GroupBy(p => PropertyUtility.GetAttribute<ButtonAttribute>(p) != null ? PropertyUtility.GetAttribute<ButtonAttribute>(p).Name : PropertyUtility.GetAttribute<BoxGroupAttribute>(p).Name);
        }
