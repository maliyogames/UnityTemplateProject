...
namespace Coffee.UIEffects
{
    ...
    public class UIEffect : BaseMaterialEffect, IMaterialModifier
    {
        ...
        public void TweenIn(float duration = .25f)
        {
            UIEffect[] m = GetComponentsInChildren<UIEffect>();
            effectFactor = 0;
            DOTween.To(() => effectFactor, x => effectFactor = x, 1, duration).OnUpdate(() =>
            {
                for (int i = 0; i < m.Length; i++)
                {
                    m[i].effectFactor = effectFactor;
                }
            });

        }

        public void TweenOut(float duration = .25f)
        {
            //effectFactor = 0;
            UIEffect[] m = GetComponentsInChildren<UIEffect>();
            DOTween.To(() => effectFactor, x => effectFactor = x, 0, duration).OnUpdate(() =>
            {
                for (int i = 0; i < m.Length; i++)
                {
                    m[i].effectFactor = effectFactor;
