...
namespace Coffee.UIEffects
{
    ...
    public class UIShadow : BaseMeshEffect, IParameterTexture
    {
        ...
        [Header("Animation")]
        [SerializeField] private bool m_enablePulsing;
        [SerializeField] private float m_pulsingSpeed = 0.0008f;
        ...
        public bool EnablePulsing { get => m_enablePulsing; set => m_enablePulsing = value; }
        ...
        protected override void OnEnable()
        {
            ...
            if (Application.isPlaying && m_enablePulsing)
                PV = StartCoroutine(Pulsate());
        }
        ...
        protected override void OnDisable()
        {
            ...
            paramTex = null;
            if (PV != null && m_enablePulsing)
                StopCoroutine(PV);
        }
        IEnumerator Pulsate()
        {
            Color tempColor = effectColor;
            while (true)
            {

                tempColor.a = Mathf.Clamp((pulse(Time.realtimeSinceStartup * m_pulsingSpeed) * maxAlpha), minAlpha, maxAlpha);
                effectColor = tempColor;
                yield return null;
            }
        ...
        float pulse(float time)
        {
            ...
            const float frequency = 1000; // Frequency in Hz
            return (float)(0.5 * (1 + Mathf.Sin(2 * pi * frequency * time) + minAlpha));
        }
